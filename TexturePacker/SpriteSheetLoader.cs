...
namespace MountainMeadowEngine.TexturePacker {
  ...
  public class SpriteSheetLoader {
...
#if __IOS__
		private readonly bool supportRetina = false;
#endif
    ...
    public SpriteSheet MultiLoad(string imageResourceFormat, int numSheets) {
      SpriteSheet result = new SpriteSheet(imageResourceFormat);
      for (int i = 0; i < numSheets; i++) {
        string imageResource = (i > 0) ? imageResourceFormat + "-" + i : imageResourceFormat;
    ...
    public SpriteSheet Load(string imageResource) {
      ...
      var sheet = new SpriteSheet(imageResource);
