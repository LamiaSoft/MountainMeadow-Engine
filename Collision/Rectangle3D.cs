...
namespace MountainMeadowEngine.Collision {
  ...
  public class Rectangle3D {
    ...
    public void SetRectangleType(RectangleTypes type) {
      this.type = type;
    }
    ...
    */
    ...
    public bool Contains(List<Tuple<int, Vector3>> coords, out List<Tuple<int, Vector3>> hits, bool debug = false) {
      hits = new List<Tuple<int, Vector3>>();

      for (int i = 0; i < coords.Count; i++) {
        if (Contains(coords[i].Item2.X, coords[i].Item2.Y, debug)) {
    ...
    public bool Contains2(float x, float y, float z) {
      bool result = false;

      result = (x >= Left && x <= Right && y >= Back && y <= Front && z >= Top && z <= Bottom);
      if (type == RectangleTypes.REGULAR) {
        return result;
      }
    ...
    public bool Contains(float x, float y, bool debug) {
      bool result = false;

      result = (x >= Left && x <= Right && y >= Back && y <= Front);
      if (!result || !HasXYSlope()) {
        return result;
      }
      ...
      */
      ...
      if (type == RectangleTypes.XYSLOPE_UP_LEFT || type == RectangleTypes.XYSLOPE_DOWN_RIGHT) {
        absPos = x - Left;
        ratio = (type == RectangleTypes.XYSLOPE_DOWN_RIGHT) ? (absPos / Length) : 1 - (absPos / Length);
        return (y <= Back + (ratio * Width));
      }
    ...
    public bool RegularOverlaps2(Rectangle3D value) {
    ...
    public bool RegularOverlaps(Rectangle3D value) {
      return (Right >= value.Left && Left <= value.Right &&
              Back <= value.Front && Front >= value.Back &&
              Top <= value.Bottom && Bottom >= value.Top);
      /*
      return (Left <= value.Right && Right >= value.Left &&
              Back <= value.Front && Front >= value.Back &&
              Top <= value.Bottom && Bottom >= value.Top);
    ...
    public void SetPosition(float? x, float? y, float? z) {
      if (x != null) {
        this.X = (float)x + Offset.X;
      }
      if (y != null) {
        this.Y = (float)y + Offset.Y;
      }
      if (z != null) {
        this.Z = (float)z + Offset.Z;
      }
    }
